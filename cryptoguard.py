...
def compute_score(res: Dict) -> int:
    # Score out of 100
    # Length component: up to 35 points (linear up to 20 chars)
    length = res["length"]
    length_score = min(length, 20) / 20 * 35
    # Diversity component: up to 30 points (lower, upper, digit, symbol)
    diversity = sum([res["has_lower"], res["has_upper"], res["has_digit"], res["has_symbol"]])
    diversity_score = (diversity / 4) * 30
    # Entropy component: up to 35 points (scaled from 0..120 bits)
...
def parse_args():
    p = argparse.ArgumentParser(prog="cryptoguard", description="Cryptoguard — password strength analyzer (CLI)")
    p.add_argument("-p", "--password", help="Password to analyze. If omitted, reads from stdin.", default=None)
    p.add_argument("--json-out", type=Path, help="Write JSON output to file")
...
def main():
    ...
    # Compute score and meter
    score = compute_score(res)
    meter = strength_meter(score, length=50)

    # Print summary (mask password display)
    masked = ('*' * (len(res['password']) - 2)).rjust(len(res['password']), '*') if len(res['password'])>2 else '*'*len(res['password'])
    print()
    print(f\"[+] Password: {masked}\")
    print(f\"[+] Length: {res['length']}\")
    contains = []
    if res['has_lower']: contains.append('lower')
    if res['has_upper']: contains.append('upper')
    if res['has_digit']: contains.append('digits')
    if res['has_symbol']: contains.append('symbols')
    ...
    if args.json_out:
        try:
            args.json_out.write_text(json.dumps({'summary': res, 'score': score}, indent=2), encoding='utf-8')
